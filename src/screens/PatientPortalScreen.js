...
const PatientPortalScreen = ({ navigation }) => {
  ...
  const handleSave = async () => {
    ...
    if (Typedocument === "Cédula") {
      const isCedulaRegistrada = await verificarCedulaRegistrada(
        documentNumber
      );
      if (isCedulaRegistrada) {
        Alert.alert(
          "Registro fallido",
          "La cédula ya se encuentra registrada. Por favor, inicia sesión."
        );
      ...
      <ScrollView>
        ...
        {isFirstTime ? (
          ...
          <Text style={styles.welcome}>
            Bienvenido, {Name} {FullName}!
          </Text>
        )}
        {cedulaRegistrada && (
          <Text style={styles.errorText}>
            La cédula ya está registrada. Por favor, inicia sesión.
          </Text>
        )}
        <Button
